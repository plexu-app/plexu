...
18-revisada. **Fases de preenchimento** do campo (substitui a fase de origem única da 18 original). `fields.config.fill_phases` (uuid[], opcional) lista as fases onde o campo é preenchido: antes da primeira fase da lista o campo fica oculto; nas fases listadas, editável; nas outras, somente leitura. `fields.config.editable_everywhere` (padrão false) o torna editável em toda fase a partir da primeira. `field_phase_settings` continua como override fino, atributo por atributo. Campo sem fases mantém o comportamento de sempre (visível e editável em todas as fases); fases arquivadas são ignoradas. Em campos calculados, a primeira fase é onde passam a ser exibidos. Obrigatório vale para sair da fase (e para criar nela ou depois dela). No card, os campos de fases anteriores aparecem em leitura na coluna "Fases anteriores", com "editar" quando a edição é permitida. Motivo da revisão: um campo pode ser preenchido em mais de uma etapa (ex.: revisão na cotação e na entrega) e alguns dados precisam de correção depois (ex.: contratante). Migração: `origin_phase_id` → `fill_phases = [origem]` (0002_fill_phases.sql).
